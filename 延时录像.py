...
class TIMELAPSE_OP(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if is_start:
            ...
            instance = TIMELAPSE(Timelapse.is_showoverlay, Timelapse.camera, Timelapse.trans_animation, Timelapse.lens_animation, Timelapse.looptype, Timelapse.start, Timelapse.end,     Timelapse.path, Timelapse.type, Timelapse.fps, Timelapse.rate, Timelapse.width, Timelapse.height)
...
def get_output_path(path):
    ...
    suffix = ".mp4" if Timelapse.type == "mp4" else ".avi"
    count = 1
    
    path_end = os.path.join(path, name_bef + suffix)
    if not os.path.exists(path_end) or Timelapse.is_overridefile:
        return path_end
...
image_old = None 
class TIMELAPSE:
    def __init__(self, is_showoverlay, camera, trans_animation, lens_animation, looptype, start, end,     path, type, fps, rate, width, height):
...
class Vars(bpy.types.PropertyGroup):
    ...
    is_overridefile : BoolProperty(name="覆盖文件",description="是否覆盖同名文件",default = True)
    is_showoverlay : BoolProperty(name="显示叠加层",description="是否显示叠加层",default = True)
